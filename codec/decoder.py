...
from ..util import parse_big_integer_to_int
from ..constants import response_status_message
    ...
    * null
    ...
    def read_boolean(self):
        ...
        if value == ord('T'):
            ...
            raise RPCConnError('illegal boolean value: {0}'.format(value))
    ...
    def read_double(self):
        ...
        if value == 0x5b:
            ...
            raise RPCConnError('{0} is not a float'.format(value))
        return result
    ...
    def read_object(self):
        ...
        if path == 'java.math.BigDecimal':
            result = float(result['stringCache']) or 0
            self.objects[-1] = result
        elif path == 'java.math.BigInteger':
            result = parse_big_integer_to_int(result)
            self.objects[-1] = result
    ...
    def read_type(self):
        ...
        if isinstance(_type, int):
            ...
            raise RPCConnError('Unknown _type type for value: {0}'.format(_type))
    ...
    def read_long(self):
        ...
        if 0xd8 <= value <= 0xef:
            ...
            raise RPCConnError('{0} is not long type'.format(value))
        return result
    ...
    def read_null(self):
        ...
        if value == ord('N'):
            ...
            raise RPCConnError('{0} is not null'.format(value))
    ...
    def read_map(self):
        ...
        if value == ord('M') or value == ord('H'):
            ...
            raise RPCConnError('{0} is not a map.'.format(value))
    ...
    def read_date(self):
        ...
        if value == 0x4a:
            ...
            raise RPCConnError('{0} is not date type'.format(value))
...
def parse_response_head(response_head):
    ...
    :return: heartbeat 0 正常响应、1 心跳响应、2 心跳请求
    """

    if not (response_head[0] == 0xda and response_head[1] == 0xbb):
        raise RPCConnError('响应头错误: %s' % str(response_head))
    ...
    if response_head[2] & 0x20 == 0x20:
        if response_head[2] & 0x80 == 0x80:
            ...
            if response_status != 20:
                raise RPCConnError(response_status_message[response_status])
    else:
        heartbeat = 0
        response_status = response_head[3]
        if response_status != 20:
